...
function generate_content_nav( $nav_id ) {
	...
	<nav id="<?php echo esc_attr( $nav_id ); ?>" class="<?php echo $nav_class; ?>">
		...
		<?php if ( is_single() ) : // navigation links for single posts 
			...
			previous_post_link( '<div class="nav-previous"><span class="prev" title="' . __('ТУДИ','generatepress') . '">%link</span></div>', '%title', $category_specific );
			next_post_link( '<div class="nav-next"><span class="next" title="' . __('СЮДИ','generatepress') . '">%link</span></div>', '%title', $category_specific );

		elseif ( is_home() || is_archive() || is_search() ) : // navigation links for home, archive, and search pages

			if ( get_next_posts_link() ) : ?>
				<div class="nav-previous"><span class="prev" title="<?php _e('ТУДИ','generatepress');?>"><?php next_posts_link( __( 'СЮДИ', 'generatepress' ) ); ?></span></div>
			<?php endif;

			if ( get_previous_posts_link() ) : ?>
				<div class="nav-next"><span class="next" title="<?php _e('ТУДИ','generatepress');?>"><?php previous_posts_link( __( 'СЮДИ', 'generatepress' ) ); ?></span></div>
			<?php endif;
			
			if ( function_exists( 'the_posts_pagination' ) ) {
				the_posts_pagination( array(
					'mid_size' => apply_filters( 'generate_pagination_mid_size', 1 ),
					'prev_text' => apply_filters( 'generate_previous_link_text', __( '&larr; ТУДИ', 'generatepress' ) ),
					'next_text' => apply_filters( 'generate_next_link_text', __( 'СЮДИ &rarr;', 'generatepress' ) ),
				) );
			}
	...
	<li id="comment-<?php comment_ID(); ?>" <?php comment_class( empty( $args['has_children'] ) ? '' : 'parent' ); ?>>
		<article id="div-comment-<?php comment_ID(); ?>" class="comment-body">
			<footer class="comment-meta">
				...
				<p class="comment-awaiting-moderation"><?php _e( 'Ваш коментар проходить модерацію.', 'generatepress' ); ?></p>
				<?php endif; ?>
			</footer><!-- .comment-meta -->
	...
	if ( $author ) {
		echo apply_filters( 'generate_post_author_output', sprintf( ' <span class="byline">%1$s</span>',
			sprintf( '<span class="author vcard" itemtype="http://schema.org/Person" itemscope="itemscope" itemprop="author">%1$s <a class="url fn n" href="%2$s" title="%3$s" rel="author" itemprop="url"><span class="author-name" itemprop="name">%4$s</span></a></span>',
				__( 'автор:','generatepress'),
				esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ),
				esc_attr( sprintf( __( 'всі теми автора %s', 'generatepress' ), get_the_author() ) ),
...
function generate_entry_meta() 
{
	...
	if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) && $comments ) {
		echo '<span class="comments-link">';
			comments_popup_link( __( 'залишити коментар', 'generatepress' ), __( '1 коментар', 'generatepress' ), __( '% коментарів', 'generatepress' ) );
		echo '</span>';
	}
}
endif;
